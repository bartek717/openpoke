...
Open-source multi-agent assistant (FastAPI backend + Next.js frontend) that orchestrates email triage (Gmail via Composio) and reminders. An **Interaction Agent** (Codex 3.5 Sonnet via OpenRouter) handles user queries and delegates to **Execution Agents** for tasks like drafting emails and scheduling reminders.
